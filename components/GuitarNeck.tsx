...
export const GuitarNeck = () => {
  ...
  const { keySig } = useKeyContext()
  useEffect(() => {
    console.log(keySig.cagedPositions)
  }, [keySig])
  const fretPositions = [
    0,
    1,
...
const Fret = ({ fretPosition }: { fretPosition: number }) => {
  ...
  return (
    ...
    >
      <div className="relative border-white border-x  border-b-2 border-b-slate-300  border-t-slate-300 w-full h-3 flex flex-row justify-center items-end">
        <NoteDot stringIdx={0} {...{ fretPosition }} />
      </div>
      <div className="relative border-white border-x border-b-2  border-b-slate-300  border-t-slate-300 w-full h-6 flex justify-center items-center">
        <NoteDot stringIdx={1} {...{ fretPosition }} />
      </div>
      <div className="relative border-white border-x border-b-2 border-b-slate-300  border-t-slate-300 w-full h-7 flex justify-center items-center">
        <NoteDot stringIdx={2} {...{ fretPosition }} />
        {fretPosition === 11 && <FretDot />}
      </div>
      <div className="relative border-white border-x border-b-2 border-t-slate-300 w-full h-7 flex justify-center items-center">
        <NoteDot stringIdx={3} {...{ fretPosition }} />
        {singleDotFrets.includes(fretPosition) && <FretDot />}
      </div>
      <div className="relative border-white border-x border-b-2 w-full h-7 flex justify-center items-center">
        <NoteDot stringIdx={4} {...{ fretPosition }} />
        {fretPosition === 11 && <FretDot />}
      </div>
      <div className="relative border-slate-200 border-x border-b-4  w-full h-7 flex justify-center items-center">
        <NoteDot stringIdx={5} {...{ fretPosition }} />
      </div>
      <div className="relative border-slate-200 border-x w-full h-5" />
    </div>
  )
}
...
const FretDot = () => (
  <div
    className={`absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-white z-0`}
  />
)
...
const NoteDot = ({
  ...
  return (
    ...
    >
      <div
        className={`rounded-full w-5 h-5 translate-y-3 ${getBGColor()} flex items-center justify-center`}
      >
